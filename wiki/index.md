...
| `wiki/pages/architecture/rust-bedrock-mds-foundation.md` | architecture | Rust-first Bedrock 第一阶段 MDS foundation 的 scope、workspace/crate 边界、reconstruction/transport contract、instrument registry、venue/live/MDS routing/fanout/envelope/wire draft 和验证标准。 | 2026-05-24 |
...
| `wiki/pages/architecture/rust-venue-sequence-rules.md` | architecture | Binance/Bitget live L2 feed 的 snapshot、delta、sequence、gap/rebuild 合同，以及 Rust sequence policy、fixtures、parser、normalization、bounded live session、MDS live routing、keyed stale、envelope、wire draft 和 in-proc fanout。 | 2026-05-24 |
